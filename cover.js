(function(root,factory){
  ...
  const positions=['main-story','bottom-story','side-story-up','side-story-down','side-story-right','column-stories'];
  ...
  function select(articles){
    ...
    const selected=positions.slice(0,5).map(position=>published.find(a=>a.homepage_position===position));
    ...
    return {hero:selected[0],secondary:selected[1],mids:selected.slice(2,4).filter(Boolean),extra:selected[4],rails:remaining.slice(0,4)};
  }

...
function createRenderer({articles = [], countries = [], routes, image = defaultImage, authorUrl} = {}) {
  ...
  function article(value) {
    ...
    return `<div class="article-kicker">${articleMeta(value, true)}</div><h1>${escapeHtml(value.title)}</h1><div class="article-dek">${escapeHtml(value.dek)}</div><div class="article-meta">BY ${byline} &nbsp; · &nbsp; ${date} &nbsp; · &nbsp; DEADLINE JOURNAL</div><figure class="article-hero">${heroImage(value, {priority:true, sizes:'(max-width: 860px) calc(100vw - 36px), 820px'})}${caption(value.hero_caption, value.hero_credit)}</figure><div class="body">${typeof value.body === 'string' ? value.body : ''}${sections(value)}</div>${sources(value)}`;
  }

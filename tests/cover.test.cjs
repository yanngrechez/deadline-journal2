...
test('named slots stay fixed while columns are newest first',()=>{
 const a=[story('main','main-story'),story('bottom','bottom-story'),story('up','side-story-up'),story('down','side-story-down'),...Array.from({length:6},(_,i)=>story('column-'+i,'column-stories',i+2))];
 const page=select(a.reverse());assert.equal(page.hero.slug,'main');assert.equal(page.secondary.slug,'bottom');assert.deepEqual(page.mids.map(a=>a.slug),['up','down']);assert.deepEqual(page.rails.map(a=>a.slug),['column-5','column-4','column-3','column-2']);
});
test('new assignment wins collisions, old assignment returns to columns, drafts never appear',()=>{
 const page=select([story('old','main-story',3),story('new','main-story',5),story('bottom','bottom-story'),story('up','side-story-up'),story('down','side-story-down'),story('draft','main-story',7,'draft')]);

(function(root){
  ...
  const reserved=Object.freeze(['index','about','write','article','region','country','search','latest','404','api','admin','assets','media','flags','content','functions','dist','node_modules','robots','sitemap','favicon','data','script','styles','analytics','routes','transition-boot','countries-data','world-map-data',...Object.values(regions)]);
  ...
  function resolve(url,articles=[]){
    ...
    if(key==='/country')return {kind:'country',code:url.searchParams.get('country')};
  ...
  const routes={regions,reserved,pathKey,regionName,articleUrl,regionUrl,resolve};

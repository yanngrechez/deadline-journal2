...
function decorate(html,{route,title,description,index=true,article,type='WebPage'}){
 ...
 if(article&&index){
  const entry={'@type':'Article','@id':url+'#article',headline:article.title,description:desc,url,mainEntityOfPage:{'@id':page['@id']},inLanguage:'en',author:{'@type':'Person',name:article.author},publisher:{'@id':organization['@id']},datePublished:date(article.published_at),articleSection:[article.region,...article.topics].filter(Boolean)};
...
module.exports={decorate,plain,date,escape};
